...
def resolve_query_template_placeholders(
    ...
    regex_pattern = "".join([project_placeholder, dataset_placeholder, r"[\w]*"])
    all_occurrences = re.findall(regex_pattern, query_template)
    tables_present_in_template = [x.replace(placeholder, "") for x in all_occurrences]

    for table in tables_present_in_template:
        dataset_table_placeholder_to_replace = "".join([dataset_placeholder, table])
        ...
        query_template = (
            ...
            .replace(dataset_table_placeholder_to_replace, new_dataset_table_name)
...
class ViewTemplate:
    ...
    def substitute(self, project: str, default_dataset: str, view_to_dataset_mapping: dict) -> str:
